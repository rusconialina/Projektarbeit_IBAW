...
app.use(function (req, res, next) {
  ...
  if (req.url !== loginRoute) {
    ...
    if (!sendAccessToken && sendAccessToken !== "") {
      // check is there access token in http authorization header
      return res
        .status(403)
        .json({ error: "Missing HTTP authorization header" });
    }

    // validate the access token
    if (!checkAccessToken(sendAccessToken)) {
      return res.status(403).json({
        error: `Permission denied. The access token:${sendAccessToken} is not valid`,
      });
    }
  }
...
const intervallInSecondsCheckIsDateExpired = 10;
...
io.on("connection", (socket) => {
  ...
  setInterval(
    checkAllBooksIsDateExpired,
    intervallInSecondsCheckIsDateExpired * 1000,
    socket
  );
});
